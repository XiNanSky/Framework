...
namespace EditorMnager
{
    ...
    public class ShaderEditor
    {
        ...
        public static bool step2(int instanceID, int line)
        {
            ...
            var strFileName = System.IO.Directory.GetParent(Application.dataPath) + "/" + strFilePath;
            if (strFileName.EndsWith(".shader"))    //文件扩展名类型
            {
                if (vscode == null || System.Diagnostics.Process.GetProcessesByName("Code").Length <= 0)
                {
                    ...
                    vscode.StartInfo.FileName = @"I:\Microsoft VS Code\Code.exe";
                    if (!File.Exists(vscode.StartInfo.FileName))
                        vscode.StartInfo.FileName = @"H:\Work\Microsoft VS Code\Code.exe";
                }
                if (vscode != null)
                {
