...
namespace Framework
{
    ...
    /// 例如当前执行函数的局部变量对象A是一个root object，他的成员变量引用了对象B，则B是一个reachable object。从roots出发可以创建reachable objects graph，剩余对象即为unreachable，可以被回收 。
    /// 
    /// GC并不是能释放所有的资源。它不能自动释放非托管资源。
    /// GC并不是实时性的，这将会造成系统性能上的瓶颈和不确定性。 对应写法 using
...
　　  10、GC对不同代的对象执行不同的检查策略以优化性能。每个GC周期都会检查第0代对象。
    ...
    */
    ///<see cref="https://docs.microsoft.com/zh-cn/dotnet/api/system.gc?view=net-5.0"/>
