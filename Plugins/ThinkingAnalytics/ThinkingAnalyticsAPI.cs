...
namespace ThinkingAnalytics
{
    ...
    public class ThinkingAnalyticsAPI : MonoBehaviour, TaExceptionHandler
    {
        ...
        public struct Token
        {
            ...
            /// <summary>
            /// 如果您使用的是云服务，请输入以下 URL: https://receiver.ta.thinkingdata.cn
            /// 如果您使用的是私有化部署的版本，请输入以下 URL: https://数据采集地址
            /// </summary>
            public string serverUrl;

            /// <summary>
            /// SDK 实例运行模式，生成环境请务必使用 NORMAL 模式。详情请参考 SDK 模式。
            /// </summary>
            public TAMode mode;

            /// <summary>
            /// 数据默认的对齐时区。如果您指定了对齐时区，那么数据的时间以及属性中的 DateTime 类型都将按照您指定的时区来对齐。
            /// </summary>
        ...
        {
            DEFAULT = 1,//2G, 3G, 4G, 5G 及 WIFI 环境下上报数据
            WIFI = 2,   //只在 WIFI 环境下上报数据
            ALL = 3     //2G, 3G, 4G, 5G 及 WIFI 环境下上报数据
        }
        ...
        /*每一个 Token 标识一个实例。如需向多个项目上报数据，可点击右下角 "+" 号增加项目配置，
         *您可以添加拥有不同的 APP ID 的多个 Token 配置。*/
        [HideInInspector]
